...
var MovieModel = require('../app/models/movie.model');  
...
describe('movie model', function() {
  ...
  it('should fail on empty data', function(done){
    ...
    promise.catch(function(err) {
      ...
      expect(err.errors.images).to.exist;
      done();
    });
  });

  it('should populate with the proper data', function(){
    var movie = new MovieModel(movieData);
    expect(movie.id).to.equal(movieData.id);
    expect(movieData).to.have.property('contents').but.not.own.property('dateViewed');
    expect(movie).to.have.property('dateViewed');
  });
